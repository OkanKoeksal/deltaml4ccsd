#!/usr/bin/env python3
"""
Central wrapper for SOAP-SVR Delta-ML models.

Usage:
    PYTHONPATH=src python -m deltaml4ccsd.train_soap \
        --config src/deltaml4ccsd/configs/BPFF_CCSD_MP2_soap.json
"""

import argparse
import runpy
from pathlib import Path

from .common import load_config, require_file


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", required=True)
    args = parser.parse_args()

    config, dataset_dir = load_config(args.config)
    dataset_dir = Path(dataset_dir)

    script_path = dataset_dir / "1_train_soap_delta_ml_models.py"
    require_file(script_path)

    print("Running SOAP workflow:")
    print("  System:      ", config["system_label"])
    print("  Formula:     ", config.get("system_formula", config["system_label"]))
    print("  Low level:   ", config["low_level_label"])
    print("  Species:     ", config.get("soap_species"))
    print("  Dataset dir: ", dataset_dir)
    print("  Script:      ", script_path)
    print()

    old_cwd = Path.cwd()

    try:
        import os
        os.chdir(str(dataset_dir))
        runpy.run_path("1_train_soap_delta_ml_models.py", run_name="__main__")
    finally:
        os.chdir(str(old_cwd))


if __name__ == "__main__":
    main()