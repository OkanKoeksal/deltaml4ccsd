...
The pretrained `.pkl` files included in this repository are the reference models used to generate the prediction files and candidate selections. When these files are present, the scripts load them automatically and reproduce the corresponding model predictions. In future versions, the retraining workflow can be made fully deterministic by explicitly seeding the Optuna sampler, running Optuna sequentially, and storing the selected hyperparameters separately from the trained model files.
...
@article{Koeksal2026DeltaMLCCSD,
  author  = {Koeksal, O.},
  title   = {Δ-Machine Learning toward CCSD Accuracy for Homohalogenated Borane-Phosphine Adducts: Screening Low-Energy Structures from DFT and MP2 Libraries},
