...
# =====================================================
# User controls: SOAP-SVR candidate structure selection
# =====================================================
...
# For BPBrBr / MP2 -> CCSD SOAP workflow:
#   N_TRAIN = 800
#   N_KNOWN = 972
#   prediction line 1   -> dataset row 800
#   prediction line 172 -> dataset row 971
LINE_OFFSET = 800

# Compare first 172 prediction rows against CCSD rows 800:972.
N_COMPARE = 172

# Predictions produced by the clean SOAP-SVR training script.
PREDICTION_FILES = {
    "SOAP_SVR": "Predictions_SOAP/predicted_ccsd_from_mp2_soap_svr.txt",
}

# If None, measured MAE from the known comparison block is used as the energy window.
...
OUTPUT_PLOTS_DIR = "Plots_HeldOut_Set_SOAP"
...
def main():
    ...
    with open(METRICS_SUMMARY, "w") as metrics_file:
        metrics_file.write(
            "File_Name\tModel\tDescriptor\tR2\tMSE\tMAE_measured\tStdErr\t"
        ...
        for model_tag, pred_path in prediction_files:
            ...
            metrics_file.write(
                f"{name_wo_ext}\t{model_tag}\t{DESCRIPTOR_LABEL}\t"
                f"{r2:.6f}\t{mse:.10e}\t{mae_measured:.10e}\t"
                f"{stderr:.10e}\t{mae_window:.10e}\t"
                f"{most_negative_value:.10f}\t{lower:.10f}\t"
                f"{upper:.10f}\t{hits_count}\n"
            )
            ...
            values_out = f"Values_Within_Range_{model_tag}_{DESCRIPTOR_LABEL}_{LOW_LEVEL_LABEL}.txt"
            ...
            target_dir = os.path.join(
                FOUND_DIR,
                f"{model_tag}_{DESCRIPTOR_LABEL}_{LOW_LEVEL_LABEL}",
            )
