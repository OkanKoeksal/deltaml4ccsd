...
# Accepted-paper / selected random state for this case.
RANDOM_STATE = 9
...
# Optuna trials are used only if models do not already exist.
GBR_TRIALS = 96
SVR_TRIALS = 96
GPR_TRIALS = 96
...
# Optional legacy names from previous runs.
LEGACY_GBR_MODEL_FILE = "final_gb_model_9.pkl"
LEGACY_SVR_MODEL_FILE = "final_svr_pipeline_9.pkl"
LEGACY_GPR_MODEL_FILE = "final_gpr_pipeline_9.pkl"
...
def load_clean_or_legacy_model(clean_path, legacy_path, label):
    if os.path.exists(clean_path):
        print(f"[{label}] Loading existing model: {clean_path}")
    ...
    if legacy_path and os.path.exists(legacy_path):
        ...
        print(f"[{label}] Also saved clean model name: {clean_path}")
        return model
    ...
    def objective(trial):
        ...
        for tr_idx, va_idx in kf.split(X_train_scaled):
            ...
            if USE_WEIGHTS_IN_FIT:
                ...
                gb.fit(X_train_scaled[tr_idx], y_train_delta[tr_idx])

            delta_pred = gb.predict(X_train_scaled[va_idx])
