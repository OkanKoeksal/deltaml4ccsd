...
# Energies
# -----------------------------
mp2_energies = np.loadtxt("DFT.txt")[:5493]
...
X_unseen_raw    = features[800:5493]    # for final predictions (same slice as remaining + more)
...
for i in range(num_random_states):
    ...
    svr_socassi_pred_unseen = mp2_energies[800:5493] + svr_pred_delta_unseen
    ...
    gpr_socassi_pred_unseen = mp2_energies[800:5493] + gpr_pred_delta_unseen
    ...
    # Predict for unseen (800-5493) using the scaler fit on train
    predicted_delta_unseen = final_gb_model.predict(X_unseen_all)
    socassi_pred_unseen_gbr = mp2_energies[800:5493] + predicted_delta_unseen
