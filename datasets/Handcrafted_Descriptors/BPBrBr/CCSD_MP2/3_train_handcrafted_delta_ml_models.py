...
FEATURES_FILE = "processed_features_final_reordered.txt"
XYZ_LIST_FILE = "processed_xyz_files_reordered.txt"
LOW_LEVEL_ENERGIES_FILE = "MP2_ML_Final_reordered.txt"
CCSD_ENERGIES_FILE = "CCSD_ML_Final_reordered.txt"

# Fallback to original names if reordered files are not present.
FALLBACK_FEATURES_FILE = "processed_features_final.txt"
FALLBACK_XYZ_LIST_FILE = "processed_xyz_files_final.txt"
FALLBACK_LOW_LEVEL_ENERGIES_FILE = "MP2_ML_Final.txt"
FALLBACK_CCSD_ENERGIES_FILE = "CCSD_ML_Final.txt"

N_TOTAL = 5557
N_CCSD = 972
N_TRAIN = 800

# Use 4 if these are the accepted-paper models.
RANDOM_STATE = 4
...
GBR_MODEL_FILE = "final_gbr_delta_ccsd_mp2.pkl"
SVR_MODEL_FILE = "final_svr_delta_ccsd_mp2.pkl"
GPR_MODEL_FILE = "final_gpr_delta_ccsd_mp2.pkl"

# Legacy model names from previous runs.
LEGACY_GBR_MODEL_FILE = "pretrained_gbr_delta_ccsd_mp2.pkl"
LEGACY_SVR_MODEL_FILE = "pretrained_svr_delta_ccsd_mp2.pkl"
LEGACY_GPR_MODEL_FILE = "pretrained_gpr_delta_ccsd_mp2.pkl"

RESULTS_DIR = "results_clean"
PRED_GBR_FILE = os.path.join(RESULTS_DIR, "predicted_ccsd_from_mp2_gbr.txt")
PRED_SVR_FILE = os.path.join(RESULTS_DIR, "predicted_ccsd_from_mp2_svr.txt")
PRED_GPR_FILE = os.path.join(RESULTS_DIR, "predicted_ccsd_from_mp2_gpr.txt")
...
def choose_existing_file(primary, fallback):
    ...
    if os.path.exists(fallback):
        return fallback
    raise FileNotFoundError("Neither file exists: {} nor {}".format(primary, fallback))
...
def load_or_train_gbr(X_train_scaled, y_train_delta, ccsd_train, low_train, weights_train):
    if os.path.exists(GBR_MODEL_FILE):
        print("[GBR] Loading existing clean model:", GBR_MODEL_FILE)
    ...
    if os.path.exists(LEGACY_GBR_MODEL_FILE):
        ...
        print("[GBR] Also saved clean model name:", GBR_MODEL_FILE)
        return model
...
def load_or_train_svr(X_train_raw, y_train_delta, ccsd_train, low_train, weights_train):
    if os.path.exists(SVR_MODEL_FILE):
        print("[SVR] Loading existing clean model:", SVR_MODEL_FILE)
    ...
    if os.path.exists(LEGACY_SVR_MODEL_FILE):
        ...
        print("[SVR] Also saved clean model name:", SVR_MODEL_FILE)
        return model
...
def load_or_train_gpr(X_train_raw, y_train_delta, ccsd_train, low_train, weights_train):
    if os.path.exists(GPR_MODEL_FILE):
        print("[GPR] Loading existing clean model:", GPR_MODEL_FILE)
    ...
    if os.path.exists(LEGACY_GPR_MODEL_FILE):
        ...
        print("[GPR] Also saved clean model name:", GPR_MODEL_FILE)
        return model
...
def main():
    ...
    features_file = choose_existing_file(FEATURES_FILE, FALLBACK_FEATURES_FILE)
    xyz_list_file = choose_existing_file(XYZ_LIST_FILE, FALLBACK_XYZ_LIST_FILE)
    low_level_file = choose_existing_file(LOW_LEVEL_ENERGIES_FILE, FALLBACK_LOW_LEVEL_ENERGIES_FILE)
    ccsd_file = choose_existing_file(CCSD_ENERGIES_FILE, FALLBACK_CCSD_ENERGIES_FILE)
    ...
    if len(xyz_files) < N_TOTAL:
        raise ValueError("XYZ list has fewer entries than N_TOTAL.")
    ...
    metrics = []
    metrics.append(evaluate_model("GBR", gbr_model, X_test_gbr, ccsd_test, low_test))
    metrics.append(evaluate_model("SVR", svr_model, X_test_pipe, ccsd_test, low_test))
    metrics.append(evaluate_model("GPR", gpr_model, X_test_pipe, ccsd_test, low_test))

    with open(SUMMARY_FILE, "w") as f:
        f.write("model\tmae_delta\tmae_ccsd\tweighted_mae_ccsd\ttail_mae_ccsd\tn_tail\n")
        for row in metrics:
            f.write(
                "{model}\t{mae_delta:.10f}\t{mae_ccsd:.10f}\t"
                "{weighted_mae_ccsd:.10f}\t{tail_mae_ccsd:.10f}\t{n_tail}\n".format(**row)
            )

    print("Wrote summary:", SUMMARY_FILE)

    # Predictions for rows N_TRAIN:N_TOTAL.
