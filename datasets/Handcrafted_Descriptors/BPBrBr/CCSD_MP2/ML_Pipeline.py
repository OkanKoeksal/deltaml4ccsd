...
if os.path.exists(processed_features_file):
    ...
    directory = "/cygdrive/d/Unikram/Aufnahmen_und_Vorlesungen/Forschungsarbeit_Donor_Acceptor_AI/ML_Donor_Acceptor/Final_ML_Results/Br3B_PBr3_8f_Original/Structures"
    ...
    for i in range(1, 33):
        filepath = os.path.join(directory, f"randomR1001_struct{i}.xyz")
        if os.path.exists(filepath):
            xyz_files.append(filepath)

    for i in range(1, 65):
        filepath = os.path.join(directory, f"randomR2001_struct{i}.xyz")
        if os.path.exists(filepath):
            xyz_files.append(filepath)

    for i in range(1, 101):
        filepath = os.path.join(directory, f"randomR1006_struct{i}.xyz")
        if os.path.exists(filepath):
            xyz_files.append(filepath)

    for i in range(1, 301):
        filepath = os.path.join(directory, f"randomR10001_struct{i}.xyz")
        if os.path.exists(filepath):
            xyz_files.append(filepath)
...
# Energies
# -----------------------------
mp2_energies = np.loadtxt("MP2_ML_Final.txt")[:5557]
socassi_energies_known = np.loadtxt("CCSD_ML_Final.txt")[:972]

# -----------------------------
# Define raw blocks (NO scaling yet) - avoids leakage
# -----------------------------
X_train_raw     = features[:800]
X_remaining_raw = features[800:972]     # to be split into val/test randomly
X_unseen_raw    = features[800:5557]     # for final predictions

socassi_train     = socassi_energies_known[:800]
socassi_remaining = socassi_energies_known[800:972]

mp2_train     = mp2_energies[:800]
mp2_remaining = mp2_energies[800:972]
mp2_unseen    = mp2_energies[800:5557]
...
print("Wrote ccsd_tail_weights_train.txt (tail weights for the 800 training rows)")
